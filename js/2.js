...
const translations = {
	"en": {
		...
		"send": "Send",
		"askTip": "Type message here，Click Set Key in the lower left corner！",
		...
		"thisQuota": "This day's quota",
		"freeTierTip": "valid until: ",
...
let customHostKey; // 自定义hostKey
let modelVersion; // 模型版本
let apiHost = localStorage.getItem("HOSTKEY"); // api地址
...
const checkBill = async () => {
	...
	let subRes;
	try {
		subRes = await fetch(apiHost + "v3/grants", {
			method: "POST",
			headers,
			body: JSON.stringify({
				keys: customAPIKey,
	...
	const usedData = subData.useAmount;
	const activateTime = subData.activateTime;
	const expire = subData.expire;

	return [usedData, activateTime, expire];
}
...
checkBillBtn.onclick = async () => {
	...
	try {
		...
		let [used, activateTime, expire] = res;
		if (used == null) {
			used = 0
		}
		usedQuota.textContent = used + "次";
		usedQuotaBar.style.width = used / 300 * 100 + "%";

		var expires_at = activateTime * 1 + (expire * 60 * 1000)
		let isExpired = expires_at < new Date().getTime()
		...
		if (activateTime == null) {
			quotaTitle.innerHTML = translations[locale]["freeTierTip"] + `<span style="color: #99c959} ">您还未使用</span>`;
		} else {
			accessDate = Y + M + D + h + m + s
...
const checkStorage = () => {
	...
	let remain = 52428800 - used;
	usedStorageBar.style.width = (used / 52428800 * 100).toFixed(2) + "%";
...
const initSetting = (force = false) => {
	...
	if (customAPIKey == "") {
		customAPIKey = localStorage.getItem("APIKey"); //兼容老版本
	}
	keyEle.onchange = () => {
		quotaContent.style.display = "none";
	...
	// host设置
	const hostEle = document.getElementById("hostInput");

	hostEle.onchange = () => {
		console.log("host的值是", hostEle.value)
		quotaContent.style.display = "none";
		customHostKey = hostEle.value;
		localStorage.setItem("HOSTKEY", customHostKey);
	}
	...
	userAvatar = setAvatarPre.src = setAvatar.value = localAvatar || setAvatar.getAttribute("value") || "https://xingimg.cachefly.net/file/fad6d6e3ff0c8e53f2306.png";
	setAvatar.onchange = () => {
